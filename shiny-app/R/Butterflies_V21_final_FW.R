...
db_color <- read_excel("../data/color_brightness_butterfly.xlsx")
                    ...
                    dashboardSidebar(
                      ...
                      sidebarMenu(
                        ...
                        fluidRow(useShinyjs(),
                          ...
                          column(11, 
                                  checkboxInput("day",
                                                "Rhopalocera on/off", value = TRUE),
                                  style = "margin-bottom:-25px;",
                          ),
                          column(11,
                                  checkboxInput("night",
                                                "Heterocera on/off", value = TRUE),
                          ...
                          column(11,
                                  checkboxInput("pictures",
                                                "pictures on/off", value = TRUE),
                          ...
                          column(11,
                                  checkboxInput("digitizer","notes of the digitizer on/off", value = FALSE),
                                  style = "margin-bottom:-20px;", 
                          ),
                          column(11,
                                  br(),
                                  actionBttn(
                                    inputId = "run_button",
                                    label = h5("search with this inputs"),
                                    color = "warning",
                                    style = "bordered"
                                  ),
                                  style = "margin-bottom:+20px;",
...
## family tree                     
                      fluidRow(
                        box(
                          title = "family tree" , status="success",collapsible = TRUE, collapsed = TRUE, width = 12,
...
## table                      
                      fluidRow(
                        box(
                          title = "table", status="success", collapsible = TRUE, width = 12,

                          withSpinner(DT::dataTableOutput("o_table", width = "100%", height = "auto"),
                                      type = 6, color = "#cf9206", size = 2),
                          
                          h6(strong("legend:"), br(),
                             HTML("&emsp;"), "ssp. = subspecies", HTML("&emsp;"), "f. = form", HTML("&emsp;"), "abb. = abberation", 
                             HTML("&emsp;"),"( ) = synonym", HTML("&emsp;"), "cf. = needs confirmation", br(),
                             
                             HTML("&emsp;"), "e.o. = ex ovum", HTML("&emsp;"), "e.l. = ex larva", HTML("&emsp;"), "e.p. = ex puppa",
                             HTML("&emsp;"), "Leg. = legit collector" , br(),
...
## geographic map                     
                      fluidRow(
                        box(
                          title = "geographic map", status="success", collapsible = TRUE, collapsed = TRUE, width = 12,
                          
                          h5 ("For further informations click on the cluster marker and the butterfly icons.(To show the map you need an active internet connection.)"),
...
## color brightness
                      fluidRow(
                        box(
                          title = "color brightness of butterflies in corellation to their elevation", status="success", collapsible = TRUE, collapsed = TRUE, width = 12, height = "100%",
...
## stop loading animation before pressing run button
  output$o_tree <- DT::renderDT(NULL)
  output$o_table <- DT::renderDT(NULL)
  output$o_map <- renderLeaflet({
    leaflet() %>% 
      addTiles()
  })
  output$o_color <- DT::renderDT(NULL)
...
## filter database #############################################################
    ...
    df_selected_filter <-  reactive({
      ...
      if (input$elevation[1] == 0){
        res <- res %>% filter(dplyr::between(as.numeric(elevation),as.numeric(input$elevation[1]),as.numeric(input$elevation[2])) | is.na(elevation) )
      }
      if (input$elevation[1] > 0){
        res <- res %>% filter(dplyr::between(as.numeric(elevation),as.numeric(input$elevation[1]),as.numeric(input$elevation[2])))
      }
      if(length(input$date[1]) > 0){
        res <- res %>% filter(dplyr::between(D_M_Y, as.Date(input$date[1]), as.Date(input$date[2])))
      }
      if (input$total_number[1] >= 0){
        res <- res %>% filter(dplyr::between(as.numeric(total_number_of_this_species),as.numeric(input$total_number[1]),as.numeric(input$total_number[2])))  
      }
...
## treeplot
  ...
  output$o_tree <- renderPlot(height = plot_height,
    ...
    ggraph(tree, "dendrogram") +    #, circular = TRUE ## circle-diagram, but to much data
      coord_flip() + 
      scale_y_reverse() +
      scale_x_reverse() +
        ...
        callback=JS(
          'table.on("mouseover","tr", function() {
              $(".small-img").hover(function(){
                    $(this).css("transform", "scale(15, 15)" 
                    );
...
## tables without pictures  
  ...
  if (input$pictures == FALSE & input$digitizer==FALSE){
    ...
    output$o_table <-
      DT::renderDataTable(server=FALSE,{
        DT::datatable(db_table[,c(1:4,6:8,10:17)], colnames = c("suborder", "family", "genus", "species", "german name", "location",
                                                                "land", "elevation", "date ", "date accuracy" , "total number of this species",
                                                                "FFH directive", "Red List Bavaria", "breeding", "collector"),
...
## table with notes
  if (input$pictures == FALSE & input$digitizer==TRUE){
    ...
    output$o_table <-
      DT::renderDataTable(server=FALSE,{
          DT::datatable(db_table[,c(1:4,6:8,10:17,18)], colnames = c("suborder", "family", "genus", "species", "german name", "location",
                                                                     "land", "elevation", "date ", "date accuracy" , "total number of this species",
  ...
  output$o_map <- renderLeaflet({
    ...
    leaflet(data = db_map) %>%
      ...
      addMarkers(~longitude, ~latitude, icon = ~butterflyicons[family], 
                 label = ~as.character(genus_and_species), popup = ~as.character(paste("<strong>",family, genus_and_species,"</strong>",
    ...
    if (length(input$color_butterfly_name )>0){ 
      ...
      df_selected_color <- reactive({ 
        ...
        if (input$color_height[1] >=0){
          res <- res %>% filter(dplyr::between(as.numeric(elevation),as.numeric((input$color_height[1])),as.numeric((input$color_height[2]))))
        }
        if (length(input$color[1]) >0){
          res <- res %>% filter(dplyr::between(as.numeric(color_brightness),as.numeric((input$color[1])),as.numeric((input$color[2])))) 
        }
...
## create regression line
    if(length(input$color_butterfly_name)==1){
...
## started to do:
  ## In Farbhelligkeit Populationen trennen
    ## sind getrennt finden neu erstellte auch selbst aber Werte passen immer noch nicht egal wie man sie trennt (Harz, Unterfranken, Alpen, Harz+Unterfranken)
    ## doch mal nach Male female trennen
    ## oder alles nochmal neu machen?
...
## update selectinput vllt mal hiermit: https://shiny.posit.co/r/reference/shiny/latest/updateselectinput
    ## immer noch viele Verknüpfungen aber einfach alle danach filtern (nur family, genus, species und vllt noch german name)

## phylogenetic trees

## input for the upload of user databases 